...
function Rentabilidade() {
  ...
  const rentabilidadeAnual = () => {
    ...
    let taxaMensal = Number(
      ((Math.pow(1 + taxaAnual, 1 / 12) - 1) * 100).toFixed(2),
    );

    const acumulado =
      inicialNum * Math.pow(1 + taxaMensal / 100, Number(tempo)) +
      mensalNum *
        ((Math.pow(1 + taxaMensal / 100, Number(tempo)) - 1) /
          (taxaMensal / 100));
  ...
  return (
    <main className='flex h-full flex-col gap-6 items-center justify-evenly p-24 max-sm:w-screen max-sm:gap-3 max-sm:p-1 max-sm:pt-24'>
      <ToastContainer />
      <h2 className='text-white text-xl max-sm:text-lg'>
      ...
      {!showResult ? (
        <>
          ...
          <div className='flex w-10/12 gap-3 flex-col items-center justify-center'>
            ...
            <Input
              placeholder='Digite valor em meses'
              type='number'
              prefix='Meses'
              name='tempo'
              value={tempo}
          ...
          <div className='flex w-10/12 gap-3 flex-col items-center justify-center'>
            ...
            <div className='flex w-full gap-36 justify-between max-sm:gap-2 min-w-[50%]'>
              <Input
                placeholder='0,00'
                prefix='%'
                name='rentabilidade'
                value={percentString(rentabilidade)}
        ...
        <div className='flex flex-col w-full gap-10 mt-10 items-center justify-evenly'>
          <h3 className='text-white text-lg max-sm:text-sm'>
            {`O valor total investido é de R$: ${converterParaReal(
              investimento,
            )}`}
          </h3>
          <h3 className='text-white text-lg max-sm:text-sm'>
            {`Juros ganhos no período R$: ${converterParaReal(
              valorAcumulado - investimento,
            )}`}
          </h3>
          <h3 className='text-white text-lg max-sm:text-sm'>
            {`O valor total acumlado é de R$: ${converterParaReal(
              valorAcumulado,
            )}`}
          </h3>
